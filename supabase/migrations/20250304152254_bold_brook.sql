/*
  ...
  1. Changes
    - Fix JSONB iteration in map_parameter_ids_to_codes function
    - Use jsonb_each instead of jsonb_each_text
    - Improve parameter code lookup
    
  2. Improvements
    - More robust parameter mapping
...
BEGIN
  ...
  FOR key_value IN SELECT * FROM jsonb_each(values_with_ids) LOOP
    IF param_map ? key_value.key THEN
      param_code := param_map->>key_value.key;
      IF param_code IS NOT NULL THEN
        mapped := mapped || jsonb_build_object(param_code, key_value.value);
      END IF;
    END IF;
  END LOOP;
