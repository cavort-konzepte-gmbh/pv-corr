...
-- Policies for substructure_systems
...
-- Policies for substructure_versions
...
-- Policies for substructures
CREATE POLICY "Enable read access for all users" ON substructures
  FOR SELECT TO authenticated USING (true);

CREATE POLICY "Enable insert for all users" ON substructures
  FOR INSERT TO authenticated WITH CHECK (true);

CREATE POLICY "Enable update for all users" ON substructures
  FOR UPDATE TO authenticated USING (true);

CREATE POLICY "Enable delete for admin users" ON substructures
  FOR DELETE TO authenticated
  USING (auth.jwt() ->> 'email' IN (SELECT email FROM admin_users));
