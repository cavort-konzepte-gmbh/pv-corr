/*
  # Add rating logic to parameters
  
  1. New Fields
    - `rating_logic_code` - Text field to store the JavaScript rating function code
    - `rating_logic_test_cases` - JSONB field to store test cases for validation
  
  2. Changes
    - Add rating_logic_code column to parameters table
    - Add rating_logic_test_cases column to parameters table
...
-- Add constraint to ensure test cases have valid structure
ALTER TABLE parameters
ADD CONSTRAINT rating_logic_test_cases_check CHECK (
  rating_logic_test_cases IS NULL OR (
    jsonb_typeof(rating_logic_test_cases) = 'array' AND
    NOT EXISTS (
      SELECT 1 FROM jsonb_array_elements(rating_logic_test_cases) AS test
      WHERE NOT (
        test ? 'input' AND
        test ? 'expected' AND
        (test->>'expected')::integer BETWEEN -12 AND 12
      )
    )
  )
);
