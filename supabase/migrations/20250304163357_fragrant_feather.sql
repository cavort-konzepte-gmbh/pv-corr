/*
  ...
  2. Changes
    - Add validation for rating_logic structure using trigger function
...
-- Create function to validate rating logic structure
CREATE OR REPLACE FUNCTION validate_rating_logic()
RETURNS trigger AS $$
DECLARE
  range_obj jsonb;
  min_val text;
  ...
  rating_val integer;
BEGIN
  -- Skip validation if rating_logic is null
  IF NEW.rating_logic IS NULL THEN
    RETURN NEW;
  END IF;

  -- Validate array structure
  IF jsonb_typeof(NEW.rating_logic) != 'array' THEN
    RAISE EXCEPTION 'rating_logic must be an array';
  END IF;

  -- Validate each range object
  FOR range_obj IN SELECT * FROM jsonb_array_elements(NEW.rating_logic) LOOP
    ...
    rating_val := (range_obj->>'rating')::integer;
    ...
    -- Validate min value
    IF min_val NOT IN ('impurities', 'never', 'constant', 'intermittent') THEN
...
-- Create trigger for rating logic validation
DROP TRIGGER IF EXISTS validate_rating_logic_trigger ON parameters;
CREATE TRIGGER validate_rating_logic_trigger
  BEFORE INSERT OR UPDATE ON parameters
  FOR EACH ROW
  EXECUTE FUNCTION validate_rating_logic();
